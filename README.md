...
> **Current status (2026-09-08 UTC):** Fabric SQL data and Alex sign-in are verified. The Work IQ integration and safe diagnostics are deployed on healthy revision 15. One approved Alex diagnostic analysis reached `ask` for both sources, but the adapter rejected the answer structure before parsing message locations. This does not establish that discovery found no messages. Live two-source evidence and downstream acceptance remain unverified. See the [deployment result](docs/deployment/workiq-discovery-integration-result.md) and [roadmap](docs/ROADMAP.md).
...
- A delegated Work IQ OBO client and bounded MCP integration in which `ask` discovers candidate message locations, `fetch` retrieves individual messages, and strict validation converts actual fetched text into source-statement Evidence Items. Graph-backed resource paths are internal to Work IQ; the application does not call Graph directly. Configured message IDs, sender/author identities, Team and channel remain validation bindings and are never discovery inputs. The implementation is reviewed and deployed; the first normal live attempt failed at discovery, so live cited acceptance remains pending.
