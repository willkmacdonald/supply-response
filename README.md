...
> **Current status (2026-09-07 UTC):** The canonical fictional RL-001 source is loaded and verified in Fabric SQL, and Alex's deployed sign-in passes. The reviewed local integration now uses Work IQ MCP `ask` to discover each source and `fetch` to retrieve it; deterministic validation turns only the fetched message text into evidence. Deployment and a normal live Analyze acceptance run for this MCP path are still pending. Successful live Work IQ retrieval, Foundry invocation, decision-linked execution and outcomes, populated Power BI parity, and rehearsals remain unverified. See the [roadmap](docs/ROADMAP.md).
...
- A delegated Work IQ OBO client and bounded MCP integration in which `ask` discovers candidate message locations, `fetch` retrieves individual messages, and strict validation converts actual fetched text into source-statement Evidence Items. Graph-backed resource paths are internal to Work IQ; the application does not call Graph directly. Configured message IDs, sender/author identities, Team and channel remain validation bindings and are never discovery inputs. The local implementation is reviewed; deployment and live cited acceptance remain pending.
