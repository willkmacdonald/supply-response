...
> **Current status (2026-09-06):** The canonical fictional RL-001 source is now loaded into Fabric SQL. The guarded loader passed live insert, exact-repeat no-op, and full readback checks. The updated Azure application is deployed, live readiness passes, and the browser successfully created a live showcase Case with the fixed scenario time. Analysis stopped with `INVALID_ACCESS_TOKEN`: the observed sign-in used the administrator account rather than Alex. Alex-authenticated analysis, Work IQ/Foundry invocation, decision-linked execution and outcomes, populated Power BI parity, and rehearsals remain unverified. See the [roadmap](docs/ROADMAP.md).
