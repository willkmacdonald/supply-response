...
> **Current status (2026-09-08 UTC):** Fabric SQL data and Alex sign-in are verified. Revision17 remains deployed while the approved structured Work IQ discovery change is implemented. Bounded CLI discovery/read verified both source bodies; Jordan passed validation, and the supplier's saved-ID mismatch has been reconciled locally against the exact approved email. Application MCP/OBO acceptance is still pending. See the [deployment result](docs/deployment/workiq-discovery-integration-result.md) and [roadmap](docs/ROADMAP.md).
...
- A delegated Work IQ OBO client and bounded MCP integration with strict source-statement validation. The approved replacement for answer-link discovery uses a narrow email query and a named Team/channel lookup through Work IQ entity tools, followed by individual message reads. This is structured discovery, not Copilot semantic search. Microsoft Graph backs Work IQ resource paths; the application has no direct Graph client. Configured message IDs remain validation checks, never lookup inputs or fallback fetch targets. Replacement deployment and application-authenticated acceptance are pending.
