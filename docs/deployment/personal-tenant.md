...
The live Playwright project disables trace, screenshots, and video so bearer-bearing headers cannot enter browser artifacts. Run it only after Task 18 approval, tenant provisioning, exact Foundry/corpus verification, successful read-only Work IQ citation verification as Alex, and confirmation that `/api/runtime` reports `live`, `fabric_sql`, `work_iq`, `foundry`, and Power BI available. The gate creates a fresh `showcase` Case, requires analysis within 90 seconds, five actions within 15 seconds, simulated observations within 65 seconds, and the same immutable Decision ID in Power BI within 60 seconds.
