...
modifying them. It creates a separate Supply Response resource group containing
one Container App, one project Key Vault, and one workspace-based Application
Insights resource. Normal scaling is 0–2 replicas at 0.5 vCPU/1 GiB. Set
`SUPPLY_RESPONSE_MIN_REPLICAS=1` only for a bounded rehearsal or showcase.
...
`--file` and suppresses command output.
...
placeholder on port 80 with no custom probes; exact managed-identity RBAC
propagation; Key Vault secret write; image build; then a second Bicep pass that
atomically establishes the system-identity ACR/Key Vault bindings, real image,
port 8000, and `/health` probes. Live nonsecret settings and `/health` follow.
...
Fabric SQL must then receive a contained database user for the managed identity
and the least privileges documented by the Fabric deployment procedure. Verify
the pinned Foundry agent names/versions independently. Do not grant tenant consent
or perform business actions from this script.
...
```bash
az containerapp update --resource-group rg-supply-response-demo --name ca-supply-response-demo --min-replicas 0
```
