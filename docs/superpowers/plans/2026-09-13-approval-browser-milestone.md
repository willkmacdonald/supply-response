...
- [ ] Keep this milestone approval-only: application composition processes action-planning outboxes for legacy workflow cases only. Independent Finance finalization still writes its durable pending outbox, but automatic and explicit workers skip it without claim, attempt, error, Case-status or receipt mutation; a mixed queue must continue processing eligible legacy work. Independent action retry and playback return bounded conflicts, and Case controls do not advertise either action. Later execution enablement may broaden the composed eligibility only with option-aware planning/playback acceptance.
