...
These scripts are intentionally not run at application startup. Apply them only to an explicitly selected Fabric SQL Database through an approved deployment process. Live tests remain skipped unless both `SUPPLY_RESPONSE_FABRIC_SQL_SERVER` and `SUPPLY_RESPONSE_FABRIC_SQL_DATABASE` are set.
