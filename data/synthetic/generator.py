...
def generate_dataset(
    *,
    seed: int = 42,
    supplier_count: int = 25,
    part_count: int = 250,
    bom_relationship_count: int = 1000,
    customer_order_count: int = 500,
) -> SyntheticDataset:
    ...
    suppliers = [
        Supplier(supplier_id=f"RL-SUP-{i:04d}", name=f"RL-Supplier-{i:04d}")
    ...
    part_numbers = [
        number for number in range(10000, 10000 + part_count + 1) if number != 10247
    ][:part_count]
    parts = [
        Part(
    ...
    for idx, part in enumerate(parts):
        ...
        supplier_parts.append(
            SupplierPart(
                ...
                part_id=part.part_id,
                lead_time_days=rng.randint(7, 40),
                is_primary=False,
            )
        )
    if parts:
        supplier_parts[0] = SupplierPart(
            supplier_id="RL-SUP-ALPHA",
            part_id="RL-MAT-10247",
            lead_time_days=14,
            is_primary=True,
        )
        supplier_parts[1] = SupplierPart(
    ...
    purchase_orders = [
        PurchaseOrder(
            ...
            supplier_id=supplier_parts[i % len(supplier_parts)].supplier_id,
            part_id=parts[i % len(parts)].part_id,
            plant_id=plants[i % len(plants)],
            quantity=rng.randint(500, 10000),
            due_date=base + timedelta(days=rng.randint(0, 60)),
        )
        for i in range(max(part_count // 2, 1))
    ]
    purchase_orders[0] = PurchaseOrder(
    ...
    inventory_positions = [
        InventoryPosition(
            ...
            quality_hold=rng.randint(0, 200),
    ...
    inventory_positions[1] = InventoryPosition(
    ...
    bom_components = [
        BomComponent(
            bom_id=f"RL-BOM-{i:07d}",
            product_id=products[i % product_count],
            component_part_id=bom_source_parts[
                rng.randrange(len(bom_source_parts))
            ].part_id,
            quantity_per=rng.randint(1, 4),
        )
        for i in range(bom_relationship_count)
    ]
    # Guarantee demo component traceability.
    if bom_components:
        bom_components[0] = BomComponent(
            bom_id="RL-BOM-DEMO",
    ...
    production_orders[0] = ProductionOrder(
    ...
    production_orders[1] = ProductionOrder(
    ...
    customers = [
        Customer(
            customer_id=f"RL-CUST-{i:04d}",
            name=f"RL-Customer-{i:04d}",
            priority=rng.randint(1, 5),
        )
        for i in range(max(25, customer_order_count // 20))
    ]
    customer_order_production_orders = production_orders[2:] or production_orders
    customer_orders = [
        CustomerOrder(
            customer_order_line_id=f"RL-CO-{i:07d}",
            ...
            quantity=rng.randint(25, 500),
            due_date=customer_order_production_orders[
                i % len(customer_order_production_orders)
            ].due_date,
            unit_revenue=Decimal(rng.randint(80, 300)),
            unit_margin=Decimal(rng.randint(20, 100)),
        )
        for i in range(customer_order_count)
    ]
    if customer_orders:
        customer_orders[0] = CustomerOrder(
        ...
        customer_orders[1] = CustomerOrder(
    ...
    transport_options = [
        TransportOption(
            transport_option_id="RL-TRANS-AIR",
            supplier_id="RL-SUP-ALPHA",
            plant_id="RL-PLANT-CHI",
            mode="air",
            max_quantity=3000,
    ...
    qualifications = [
        QualityQualification(
            qualification_id="RL-QUAL-ALPHA",
            supplier_id="RL-SUP-ALPHA",
            part_id="RL-MAT-10247",
...
def generate_demo_scale_dataset(*, seed: int = 42) -> SyntheticDataset:
    ...
    return generate_dataset(
        ...
        supplier_count=250,
        part_count=10_000,
        bom_relationship_count=100_000,
        customer_order_count=50_000,
    )
