...
def test_usable_inventory_formula():
    p = InventoryPosition(
        ...
        on_hand=100,
        quality_hold=10,
        protected_allocation=15,
    )
    assert calculate_usable_inventory(p) == 75


def test_projected_balance_and_stockout_are_deterministic():
    points = project_inventory(
        starting_balance=100,
        receipts=[TimedQuantity(date=date(2026, 9, 2), quantity=25, source_id="RL-R")],
        transfers=[],
        demand=[
            TimedQuantity(date=date(2026, 9, 2), quantity=80, source_id="RL-D1"),
            TimedQuantity(date=date(2026, 9, 3), quantity=60, source_id="RL-D2"),
        ],
    )
...
def test_exposure_identifies_orders_and_financial_risk():
    ...
    bom = [
        BomComponent(
            bom_id="RL-B",
            product_id="RL-PROD",
            component_part_id="RL-MAT",
            quantity_per=1,
        )
    ]
    prod = [
        ProductionOrder(
            production_order_id="RL-MO-1",
            product_id="RL-PROD",
            plant_id="RL-PLANT",
            quantity=150,
    ...
    orders = [
        CustomerOrder(
            customer_order_line_id="RL-CO-1",
            customer_id="RL-C",
            product_id="RL-PROD",
            plant_id="RL-PLANT",
            production_order_id="RL-MO-1",
            quantity=150,
            due_date=date(2026, 9, 5),
            unit_revenue=Decimal("10"),
            unit_margin=Decimal("4"),
        )
    ]
    result = calculate_exposure(
        analysis_id="RL-ANALYSIS-EXPOSURE",
        part_id="RL-MAT",
        plant_id="RL-PLANT",
    ...
    assert result.metadata.analysis_id == "RL-ANALYSIS-EXPOSURE"
